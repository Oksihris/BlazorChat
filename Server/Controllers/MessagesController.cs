...
namespace BlazorChat.Server.Controllers
{
    ...
    public class MessagesController : BaseController
    {
        ...
        public MessagesController(ChatContext chatContext, IHubContext<ChatHub, IChatHubClient> hubContext)
        {
        ...
        public async Task<IActionResult> SendMessage(MessageSendDto messageDto, CancellationToken cancellationToken)
        {
            ...
            if (await _chatContext.SaveChangesAsync(cancellationToken) > 0)
            {
                ...
                await _hubContext.Clients.User(messageDto.ToUserId.ToString()).MessageRecieved(responseMessageDto);
                return Ok();
            }
            else
            {
